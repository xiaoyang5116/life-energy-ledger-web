...
- [x] 抽屉遮罩层在 toast 之上，点击「去设置」其实是关闭抽屉遮罩层，而不是打开设置页（Toaster z-index 提高；去设置先关抽屉再导航）
...
- [ ] tabbar 待复盘 添加 badge 标记 提示待复盘数量

## P2：后续产品

- [ ] 幽灵账（突发挂起，暂不正式记入；已从录入 UI 移除，后续再做）
- [ ] 相机 / 拍照识别辅助录入（已从录入 UI 移除，后续再做）
